...
def compile_file(file_path):
    ...
    subprocess.run(
        f"multipass transfer {file_path} manager:cloud",
        shell=True
    )
    ...
    subprocess.run(
        f"multipass exec manager -- mpicc -o /home/ubuntu/cloud/{file_name} "
        f"/home/ubuntu/cloud/{full_file_name}",
        shell=True
    )
...
def run_mpi_program(file_name, num_threads):
    ...
    result = subprocess.run(
        f"multipass exec manager -- test -f /home/ubuntu/cloud/{file_name}",
        shell=True
    )
    ...
    mpi_command = "mpiexec -hostfile /home/ubuntu/cloud/mpi_hosts"

    if num_threads != -1:
        mpi_command += f" -n {num_threads}"

    subprocess.run(
        f"multipass exec manager -- {mpi_command} /home/ubuntu/cloud/{file_name}",
        shell=True
    )
...
def main():
    ...
    parser.add_argument(
        '--c', '-c',
        metavar='path_to_file.c',
        type=str,
        help='Path to the C file to compile'
    )
