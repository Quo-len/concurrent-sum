...
def launch_vms(size, cpu, ram):
    ...
    for i in range(size):
        ...
        subprocess.run(
            f"multipass launch -n {vm_name} --cpus {cpu} "
            f"--memory {ram}M --cloud-init cloud-init.yaml",
            shell=True
        )
    ...
    for name, ip in vm_ips.items():
        ...
        for other_name, other_ip in vm_ips.items():
            if name != other_name:
                ...
                subprocess.run(f"{echo_cmd}/etc/hosts\"", shell=True)
                subprocess.run(f"{echo_cmd}/etc/cloud/templates/hosts.debian.tmpl\"", shell=True)
...
def generate_ssh_keys(size):
    ...
    for i in range(size):
        ...
        subprocess.run(
            f"multipass exec {vm_name} -- ssh-keygen -q -t rsa -C {vm_name} "
            "-f /home/ubuntu/.ssh/id_rsa -N \"\"",
            shell=True
        )
...
def setup_ssh_keys(ssh_keys):
    for vm_name, _ in ssh_keys.items():
        ...
        for other_vm, other_key in ssh_keys.items():
            if vm_name != other_vm:
                subprocess.run(
                    ...
                    f"\"echo '{other_key}' >> /home/ubuntu/.ssh/authorized_keys\"",
                    shell=True
                )

                subprocess.run(
                    f"multipass exec {other_vm} -- ssh {vm_name} -q -o "
                    "StrictHostKeyChecking=no echo '{other_vm}'",
                    shell=True
                )
...
def setup_nfs_server():
    ...
    subprocess.run(
        f"{bash} -- sudo bash -c \"echo '{exports_content}' >> /etc/exports\"",
        shell=True
    )
...
def create_mpi_hosts(size):
    ...
    for i in range(size):
        vm_name = "manager" if i == 0 else f"worker{i}"
        subprocess.run(
            f"multipass exec manager -- bash -c "
            f"\"echo '{vm_name}' >> /home/ubuntu/cloud/mpi_hosts\"",
            shell=True
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Setup VMs with NFS and OpenMPI'
    )
    parser.add_argument(
        'size', metavar="S", type=int,
        help='The size of the VM cluster to create'
    )
    parser.add_argument('--cpu', type=int, default=1,
                        help='Number of CPUs for each VM (default: 1)')
    parser.add_argument('--ram', type=int, default=1024,
                        help='Amount of RAM in MB for each VM (default: 1024)')
